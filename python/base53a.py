...
class Base53ID(object):
	...
	def __init__(self, *, string_without_checksum:str, checksum_char:str, remap:bool=False, autogenerate:bool=False):
		# 1. check types
		assert type(string_without_checksum) is str, ValidationResult(False, 'Not string type')
		assert type(checksum_char) is str, ValidationResult(False, 'Not string type')
		# do the autogeneration, this is unsafe but it's useful for testing
		if autogenerate:
...
def b53_generate_next_Base53ID(old_id:Base53ID) -> Base53ID:
	...
	for _ in range(5): # try 5 times before giving up
		...
		try:
			new_id = Base53ID(string_without_checksum=new_s, checksum_char='zzz', autogenerate=True)
			return new_id
